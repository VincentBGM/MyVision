...
class VertexCameraBAL : public g2o::BaseVertex<9, Eigen::Vector3d >
{
    ...
    {
        // 由于update是个double类型数组，而增量需要的是个矩阵， 所以用update构造一个增量矩阵v，下面更新估计值时，直接将v加上
...
// 路标点节点
//定义landmark类型顶点，维度3维，类型是Eigen::Vector3d
class VertexPointsBAL : public g2o::BaseVertex<3, Eigen::Vector3d >
{
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
// 总结：
//  Eigen::VectorXd::ConstMapType v ( update, VertexCameraBAL::Dimension );  动态矩阵 相机顶点是6个维度
//  Eigen::Vector3d::ConstMapType v ( update );  对于动态矩阵 顶点是3个维度
//
...
class EdgeObservationBAL : public g2o::BaseBinaryEdge<2, Eigen::Vector2d, VertexCameraBAL, VertexPointsBAL>
{
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    ...
    virtual void computeError()
    {
        ...
        // 设第1个顶点，相机位姿取出来。
        const VertexPointsBAL* point = static_cast< const VertexPointsBAL* > (vertex(1) );
    ...
    // 为了使用自动求导 让本类成为拟函数类
    // 为重载的()函数，为模板函数，需要数据为相机位姿指针，空间点位置指针，用于承接输出误差的residuals。
    // 上面调用时，用的_error.data()承接，完成误差计算。
    // 这个模板类其实还是用的重投影误差
    template <typename  T>
    bool operator() (const T* camare, const T* point, T* residuals ) const
    {
        //这里创建一个承接重投影像素坐标，也就是根据相机内外参和空间点坐标去投影得到的像素坐标，是估计值。
        T predictions[2];

        //这个函数就是反应的投影过程，camera参数，point参数，然后用predictions承接算得的像素坐标。这里也发现就是二维的
        CamProjectionWithDistortion ( camare, point, predictions );
    ...
    virtual void linearizeOplus()
    {
        ...
        typedef ceres::internal::AutoDiff<EdgeObservationBAL, double, VertexCameraBAL::Dimension, VertexPointsBAL::Dimension> BalAutoDiff;
        ...
        Eigen::Matrix<double, Dimension, VertexPointsBAL::Dimension, Eigen::RowMajor> dError_dPoint;
...
// 总结
// 线性增量方程：linearizeOplus()
// 也就是求雅克比矩阵，之前用的都是公式计算好了，直接对雅克比矩阵各个元素赋值。这里是利用了ceres库中的AutoDiff去计算的。
// 不管怎么搞，要的结果就是把雅克比矩阵_jacobianOplusXi和_jacobianOplusXj定义出来。
// 具体看一下，说白了这个函数就是求得误差关于优化变量的导数。由于这里误差跟两个优化变量有关，所以是二元边，
// 所以求导就是两个优化变量的偏导，也就是_jacobianOplusXi和_jacobianOplusXj了。
// 相应的如果是一元边，那就只有_jacobianOplusXi，三元边那就再来个_jacobianOplusXk。
