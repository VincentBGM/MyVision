...
#ifndef G2O_BA_BALPROBLEM_H
#define G2O_BA_BALPROBLEM_H
...
public:
    ...
    int point_block_size()              const{ return 3;                         }  //返回路标维度，是3维
...
private:
     ...
     * 分法小复杂。可以画张图
     */
    int num_cameras_;            //数据中相机位姿个数
    int num_points_;            //数据数据中路标点个数
    int num_observations_;      //数据中的观测个数
    int num_parameters_;        //这个从后方定义来看是待优化变量的具体的参数的个数: num_parameters_ = 9 * num_cameras_ + 3 * num_points_;
    bool use_quaternions_;

    //这三个存储了相机与路标之间的关联
    int* point_index_;           //观测中路标的索引数组
    int* camera_index_;             //观测中相机的索引数组
    double* observations_;      //观测的索引数组
