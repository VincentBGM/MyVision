...
#ifndef G2O_BA_BUNDLEPARAMS_H
#define G2O_BA_BUNDLEPARAMS_H
...
public:
    ...
    string input;                               // 输入,那个文件被处理
    string trust_region_strategy;               // 信赖域策略
    string linear_solver;                       // 线性求解器
    string sparse_linear_algebra_library;       // 稀疏的线性代数求解器
    string dense_linear_algebra_library;        // 稠密的线性代数求解器

    bool robustify; // loss function            // 是否鲁棒??
    // double eta;
    ...
    double rotation_sigma;                      //旋转sigma
    double translation_sigma;                   // 变换sigma
    double point_sigma;                         //空间点sigma

    // for point cloud file...
    string initial_ply;                         //原始点云
    string final_ply;                           // 优化后点云
