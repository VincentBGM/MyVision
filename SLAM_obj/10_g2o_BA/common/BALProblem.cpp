...
BALProblem::BALProblem(const std::string& filename, bool use_quaternions)
{
    ...
    point_index_ = new int[num_observations_];          //.txt中相机索引那一列，存在point_index_数组，大小肯定是观测个数
    camera_index_ = new int[num_observations_];         //.txt中路标点索引那一列，存在camera_index_数组，大小肯定是观测个数
    observations_ = new double[2 * num_observations_];   //后面的观测值的话，由于每个观测是个二维的坐标，所以大小要2倍

    num_parameters_ = 9 * num_cameras_ + 3 * num_points_;   //所有要优化的参数量，相机个数*9维，路标点个数*3维。所以这里的num_parameters_就指所有要优化的值了。
    parameters_ = new double[num_parameters_];  //将这些值存进parameters_数组中。这里也就是优化变量的值，只不过全部一列码开，在txt中也就是后半部分，所有优化变量的初始值
